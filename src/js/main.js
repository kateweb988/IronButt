...
document.addEventListener('DOMContentLoaded', function () {
  ...
  $('.a2, .merch__btn').click(function (e) {
    e.preventDefault();
    $('#popup-merch').arcticmodal({
    });
  });

});
document.querySelectorAll('.custom-select').forEach(select => {
  let header = select.querySelector('.select-header');
  let headerText = header.querySelector('.header-text'); // только текст
  let defaultText = headerText.textContent; // запоминаем исходный текст
  ...
  checkboxes.forEach(cb => {
    cb.addEventListener('change', () => {
      let checked = select.querySelectorAll('input[type="checkbox"]:checked').length;
      if (checked === 0) {
        headerText.textContent = defaultText; // возвращаем исходный текст
      } else {
        headerText.textContent = `Выбрано: ${checked}`;
      }
    });
  });
});
